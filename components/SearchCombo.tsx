...
import { airlinesList } from "../src/app/airlines/page";
...
const SearchCombo = ({airline, setAirline} : props) => {
    ...
    query === "" ? airlinesList : airlinesList.filter((item) =>
    ...
    return (
        ...
        <Combobox value={airline} onChange={setAirline}>
            ...
            >
                ...
                className="absolute max-h-60 mt-3 w-full overflow-auto rounded-lg bg-gray-200 py-2 text-base shadow-xl">
                {filteredAirlinesList.length === 0 && query !== "" ? (
                    <Combobox.Option
                    value={query} 
                    className='search-airline__option'
                    >
                    ...
                    >
                        ...
                        <>
                            ...
                            {selected ? (
                            <span className={`absolute inset-y-0 left-0 flex items-center pl-3 ${active? "text-white" : " bg-violet"}`}
                            ></span>
                            ) : null}
                        </>
                        )}
