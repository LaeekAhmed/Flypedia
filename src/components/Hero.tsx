...
export default async function Component() {
  ...
  const filteredFlights = flight_data.response
    .filter((flight: FlightInfo) => flight.flight_iata)
    .slice(0, 8);
  let airline = "AC";

  return (
    <div className="flex flex-col items-center w-full min-h-screen p-4 sm:p-8 space-y-8 sm:space-y-12">
      <header className="text-center pt-4 sm:pt-8">
        <h1 className="text-3xl sm:text-5xl font-bold">
          Check flight status of world class airlines
          <br />
          <span className="text-gray-400 text-4xl sm:text-6xl">
            with Flypedia 🛫
          </span>
        </h1>
      </header>
      ...
      <div className="w-full mx-auto px-4 sm:px-8 py-8">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground">
            Track the current status of {mappings[airline]} flights.
          </p>
        </div>
